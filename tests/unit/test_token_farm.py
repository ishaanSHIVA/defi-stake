...
def test_stake_tokens(amount_staked):
    ...
    tx.wait(1)
    print("Staking balance :- ", tx.return_value)
    stakingBalance = tx.return_value
    # Assert
    assert (
...
def test_unstake_tokens(amount_staked):
    ...
    tx.wait(1)
    print("To be unstaked balance :- ", tx.return_value)
    print("to be staked balance :- ", stakingBalance)
    print(
        "stake balance and unstake balance :- ",
        int(tx.return_value) == int(stakingBalance),
    )
...
def test_token_is_allowed():
    ...
    tx = token_farm.tokenIsAllowed(dapp_token.address)
    tx.wait(1)
